...
public:
    ...
    {
        NT_ASSERTMSG(
            "offset of CallbackNodes field not set",
            _offsetCallbackNodesField != INVALID_OFFSET);
...
public:
    ...
    {
        NT_ASSERTMSG(
            "FltRegisterFilter failed",
             NT_SUCCESS(FltRegisterFilter(DriverObject, &registersion, &_object)));
    ...
    void startFiltering()
    {
        ...
        NT_ASSERTMSG("Failed to start filter", NT_SUCCESS(status));
    }
    ...
    unsigned long getInstances(ArrayGuard<FltInstanceGuard*, true>& instancesGuardArray)
    {
        ...
        NT_ASSERTMSG("Enumeration of instances by filter failed", NT_SUCCESS(status));
